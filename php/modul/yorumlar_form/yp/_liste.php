...
$yorum_sayisi = $pdo_db->var("SELECT COUNT(no) FROM {$do_}yorumlar WHERE no > 0 AND tablo != '' AND tablo IS NOT NULL");
...
try {
	...
	$stmt = $pdo->prepare("SELECT no, uye_no, tablo, tablo_no, yorum_no, adi_soyadi, email_adresi, mesaj, tarih, ip, yayin
	        FROM {$do_}yorumlar
	        WHERE no > 0 AND tablo != '' AND tablo IS NOT NULL
	        ORDER BY no DESC
	        LIMIT $limit1, $limit2");
	$stmt->execute();
	$yorumlar = $stmt->fetchAll(PDO::FETCH_OBJ);
} catch (PDOException $e) {
	error_log('PDO error in yorumlar _liste: ' . $e->getMessage());
	$yorumlar = [];
}
...
if (!empty($yorumlar)) {
	...
	foreach ($yorumlar as $s) {
		try {
			$stmt_tablo = $pdo->prepare("SELECT adi FROM {$do}_{$s->tablo} WHERE no = :tablo_no");
			$stmt_tablo->execute([':tablo_no' => $s->tablo_no]);
			$tablo_b = $stmt_tablo->fetch(PDO::FETCH_OBJ);
		} catch (PDOException $e) {
			error_log('PDO error fetching related table name in yorumlar _liste: ' . $e->getMessage());
			$tablo_b = false;
		}
		if ($tablo_b) {
			echo '
			<tr>		
			<td>'.$s->no.'</td>
			<td class="yazi1">'.($tablo_b ? $tablo_b->adi : ''). '('.$s->tablo.')</td>
			...
			echo '<tr><td colspan="8">'.$s->no.'</td></tr>';
		}
	}
	echo '</tbody></table>';
